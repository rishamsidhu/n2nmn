...
#inch.print_tensors_in_checkpoint_file(snapshot_file, 'neural_module_network/layout_execution/Variable', False)
...
with tf.variable_scope("neural_module_network/layout_execution", reuse=True):
    modules = Modules(image_feat_grid_ph, word_vecs_ph, num_choices, swaps)
...
for x in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES):
    if "layout_execution_1" not in x.name:
        var_list += [x]
...
def run_visualization(dataset_tst):
    ...
    for n, batch in enumerate(dataset_tst.batches()):
        ...
        plt.subplot(4, 3, 1)
        #print(batch['image_path_list'][0].replace("clevr-dataset", "my_dataset"))
        ...
        for t, module_name, results in all_output_stack:
            ...
            if results.ndim > 2:
                #sh = tf.shape(results[..., 0])
                dist = results[..., 0].flatten() / 3
                dist = np.exp(dist) / np.sum(np.exp(dist))
                #print(sh)
                dist = np.reshape(dist, (10, 15))
                plt.imshow(dist, interpolation='nearest', vmin=0, vmax=0.2, cmap='Reds')
                plt.axis('off')
                ...
                mods_count[module_name] += 1
                dist = np.exp(results.flatten() / 3)
                dist = dist / np.sum(dist)
                plot = np.tile(dist.reshape((1, num_choices)), (2, 1))
                plt.imshow(plot, interpolation='nearest', vmin=0, vmax=1.0, cmap='Reds')
