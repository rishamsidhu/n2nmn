...
exp_name = "clevr_gt_layout_" + input("Experiment name: ")
...
summary_trn.append(tf.summary.scalar("training_accuracy", tr_acc))
...
def run_training(max_iter, dataset_trn):
    ...
    prefix = 'neural_module_network/layout_execution/module_variables/'
    mods = ['TransformModule', 'FindModule', 'AnswerModule', 'FindSamePropertyModule', \
    'ExistModule', 'CountModule', 'EqualNumModule', 'MoreNumModule', \
    'LessNumModule', 'SamePropertyModule', 'DescribeModule']
    ...
    for n_iter, batch in enumerate(dataset_trn.batches()):
        ...
        #test
        """
        for x in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = prefix + "FindModule"):
            if "bias" not in x.name and "Adam" not in x.name:
                y = x.eval(session = sess)
                x.load(np.zeros(y.shape), session = sess)

        for x in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = prefix + "FindModule"):
            #print(x.name)
            #print("something")
            #print(x.eval(session = sess)[:10])
        """
