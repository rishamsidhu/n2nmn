...
def run_visualization(dataset_tst):
    ...
    non_final = []
    for i in range(100):
        non_final += [entropy(np.exp(most_outputs[i][j])) for j in range(len(most_outputs[i]) - 1)]
        final += [entropy(np.exp(most_outputs[i][-1]))]
    print("final entropy : ", sum(final) / len(final))
    print("non final entropy : ", sum(non_final) / len(non_final))
