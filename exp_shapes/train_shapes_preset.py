...
preset_folder = './exp_shapes/tfmodel/%s/' % input("Name of folder containing presets: ")
num_swaps = int(input("Number of swaps: "))

if num_swaps != 0:
    with open(os.path.join(preset_folder, "start_swaps.txt"), "rb") as f:
        swaps = pickle.load(f)
else:
    swaps = []

with open(os.path.join(preset_folder, "start_vars.txt"), "rb") as f:
    preset_variables = pickle.load(f)
...
#num_swaps = len(swaps['FindModule'])
...
for x in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES):
    x.load(preset_variables[x.name], session = sess)
...
for n_iter in range(max_iter):
    ...
    if n_begin == 0:
        ...
        layout_accuracy = layout_correct / num_val_questions
        layout_validity = layout_valid / num_val_questions
        print("answer accuracy =", answer_accuracy)
        print("layout accuracy =", layout_accuracy)
        print("layout validity =", layout_validity)
        all_val_accs += [answer_accuracy]
...
with open(os.path.join(snapshot_dir, "validation_acc.txt"), "wb") as f:
    pickle.dump(all_val_accs, f)
